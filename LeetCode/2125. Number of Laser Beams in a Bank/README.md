...
class Solution {
    public int numberOfBeams(String[] bank) {
        int totalLazerBeams = 0;
        int cellLazerBeams = 0;
        for(String s : bank){
            int devices = 0;
            for(char cell : s.toCharArray()){
                if(cell == '1')
                    devices++;
            }
            if(devices > 0){
                totalLazerBeams += (devices * cellLazerBeams);
                cellLazerBeams = devices;
            }
        }
        return totalLazerBeams;
    }
}
```